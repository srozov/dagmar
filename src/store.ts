...
export class Store {
  ...
  runs(): RunRow[] { return (this.db.prepare("SELECT * FROM workflow_runs ORDER BY started_at DESC").all() as Row[]).map(run); }
  attempt(id: string): AttemptRow | undefined { const row = this.db.prepare("SELECT * FROM task_runs WHERE id=?").get(id) as Row | undefined; return row && attempt(row); }
  attempts(runId: string): AttemptRow[] { return (this.db.prepare("SELECT * FROM task_runs WHERE workflow_run_id=? ORDER BY task_id,attempt").all(runId) as Row[]).map(attempt); }
  activeAttempts(): AttemptRow[] { return (this.db.prepare("SELECT * FROM task_runs WHERE status IN ('running','awaiting_permission','awaiting_input')").all() as Row[]).map(attempt); }
  nextAttempt(runId: string, taskId: string): number { return Number((this.db.prepare("SELECT COALESCE(MAX(attempt),0)+1 n FROM task_runs WHERE workflow_run_id=? AND task_id=?").get(runId, taskId) as Row).n); }
  ...
  updateRun(id: string, patch: Partial<Pick<RunRow, "status" | "updatedAt" | "endedAt">>): RunRow {
    ...
    const value = this.run(id); if (!value) throw new DagmarError("run_not_found", `Run ${id} was not found`); return value;
  }
  updateAttempt(id: string, patch: Partial<Pick<AttemptRow, "status" | "result" | "error" | "acpSessionId" | "updatedAt" | "endedAt">>): AttemptRow {
    ...
    const value = this.attempt(id); if (!value) throw new DagmarError("task_run_not_found", `Task run ${id} was not found`); return value;
  }
}
...
function run(r: Row): RunRow { return { id: text(r,"id"), workflowId: text(r,"workflow_id"), input: JSON.parse(text(r,"input_json")), status: text(r,"status") as RunRow["status"], startedAt: text(r,"started_at"), updatedAt: text(r,"updated_at"), endedAt: nullable(r,"ended_at") }; }
function attempt(r: Row): AttemptRow { return { id:text(r,"id"), workflowRunId:text(r,"workflow_run_id"), taskId:text(r,"task_id"), attempt:Number(r.attempt), executorProfile:text(r,"executor_profile"), executorType:text(r,"executor_type") as AttemptRow["executorType"], status:text(r,"status") as AttemptRow["status"], result:decode<TaskResult>(r.result_json), error:decode<TaskError>(r.error_json), acpSessionId:nullable(r,"acp_session_id"), startedAt:text(r,"started_at"), updatedAt:text(r,"updated_at"), endedAt:nullable(r,"ended_at") }; }
