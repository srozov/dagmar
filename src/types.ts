...
export interface TaskDef { executor: string; dependsOn: string[]; inputs: JsonObject; prompt?: string; run?: [string, ...string[]]; outputSchema?: JsonSchema; session?: { mode: "fresh" } | { mode: "continue"; from: string } }
...
export interface Interaction { id: string; workflowRunId: string; taskRunId: string; kind: "permission" | "input"; method: "session/request_permission" | "elicitation/create"; request: Json; createdAt: string }
