...
export interface TaskDef { executor: string; dependsOn: string[]; inputs: JsonObject; prompt?: string; run?: [string, ...string[]]; outputSchema?: JsonSchema; interactive?: boolean; session?: { mode: "fresh" } | { mode: "continue"; from: string } }
...
export interface Interaction { id: string; workflowRunId: string; taskRunId: string; kind: "permission" | "input" | "turn"; method: "session/request_permission" | "elicitation/create" | "turn/next"; request: Json; createdAt: string }
