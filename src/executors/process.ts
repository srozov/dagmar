...
export class ProcessExecutor implements Executor<ProcessRequest> {
  async start(request: ProcessRequest, hooks: Hooks): Promise<Execution> {
    ...
    try {
      ...
      return finished(failure("process_spawn_failed", error));
    }
...
function run(child: ChildProcessWithoutNullStreams, request: ProcessRequest, hooks: Hooks): Execution {
  ...
  const record = (value: Parameters<Hooks["transcript"]>[0]): void => {
    records = records.then(() => hooks.transcript(value)).then(() => undefined);
  };
  ...
  child.stderr.on("data", (chunk: string) =>
    record({ type: "stdio", direction: "from_executor", stream: "stderr", data: chunk }),
  );
  ...
  const done = (async (): Promise<Settlement> => {
    try {
      ...
      record({
        ...
        data: { code: exit.code, signal: exit.signal },
      });
      await records;
      if (cancelled) {
        return { error: { code: "cancelled", message: "Process was cancelled" } };
      }
      if (exit.code !== 0) {
        return { error: { code: "process_exit_nonzero", message: `Process exited with code ${exit.code}` } };
      }
      if (!stdout.trim()) {
        return { error: { code: "process_stdout_missing", message: "Process produced no result" } };
      }
      let value: unknown;
      try {
        ...
        return {
          error: { code: "process_stdout_invalid", message: "Process stdout must be exactly one JSON value" },
        };
      }
      try {
        return { result: validateResult(value, request.outputSchema) };
      } catch (error) {
        return failure("result_validation_failed", error);
      }
    } catch (error) {
      return failure("process_failed", error);
    }
  })();
  return {
    done,
    cancel() {
      ...
      return (cancelPromise ??= (async () => {
        ...
        if (child.exitCode === null && child.pid) {
          ...
          if (didTimeout) signal(child, "SIGKILL");
        }
        await closed;
        await done;
      })());
...
function signal(child: ChildProcessWithoutNullStreams, value: NodeJS.Signals): void {
  try {
    process.kill(-(child.pid as number), value);
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ESRCH") child.kill(value);
  }
}

function failure(code: string, error: unknown): { error: TaskError } {
  return {
    error: {
      code: error instanceof DagmarError ? error.code : code,
      message: error instanceof Error ? error.message : "Process failed",
    },
  };
}

function finished(value: Settlement): Execution {
  return { done: Promise.resolve(value), cancel: async () => undefined };
}
